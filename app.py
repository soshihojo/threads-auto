...
# ---------------- 一問鑑定（299円・手動送付のAI支援） ----------------
if view == VIEW_ONEQ:
    st.caption("299円の一問鑑定はボットは自動で売りません。ここで文面を作り、LINE公式アプリからあなたが手動で送ります。"
               "使いどころ：購入サインが出た人・本鑑定（3,280円）のオファーに反応が無い人・「聞きたいことが一つだけありそう」な人。")
    if not (profile.get("oneq_url") or "").strip():
        st.warning("⚠️ 決済リンク（oneq_url）が未設定です。STORES等で299円商品を作り、config.yaml の oneq_url に貼ってください。"
                   "それまで文面には差し替えメモが入ります。")
    ...
    st.text_area("フォロー文（コピー用の定型。状況に合わせて一言足すと良い）", height=170, key="oq_follow",
                 value=("この前の一問、どうやった？\n"
                        ...
                        "ほんでな、あの時言うた「ここから先の全体の地図」——本鑑定な、"
                        "あんたは一問の299円をそのまま引いた2,981円で視られる（専用の申し込み先を渡すで）。\n"
                        "急がんでええ、視たくなったら言うてな🌙"))
