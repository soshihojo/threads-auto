...
def generate_consult(me_birth: str, him_birth: str, message: str, history: str = "",
    ...
    user = (
        ...
        f"【今日の日付】{_today.year}年{_today.month}月{_today.day}日\n"
...
# ---------------- 一問鑑定（199円・手動送付） ----------------
# 店主がダッシュボードで生成→LINE公式アプリから手動で送る。ボットは自動送付しない。

ONEQ_OFFER_SYSTEM = """あなたは恋愛・復縁専門の占い師「椿（つばき）」。無料相談中の相手に「一問鑑定（199円）」を案内する導入メッセージだけを書く（この後ろに商品の固定案内が続く）。

声: 一人称「ウチ」、相手は「あんた」。関西弁・タメ口。毒舌7・愛3。売り込み臭・急かしは禁止。

この相談者ひとりのためのオーダーメイドで書く。構成（140〜220字・改行で読みやすく）:
1. 会話から相手の状況の具体（固有の場面・相手の言葉）を引用して、見立てをひとこと
2. 何が視えるのか: この人の悩みに合わせて「一問鑑定で何を視て、何を答えるか」を名指しで書く（例: 彼があの一言のあと黙っとる本当の理由と、あんたが次に打つ一手）。一般論は禁止、この会話の場面・言葉で書く
3. なぜ有料か: ここまでの無料の会話は、話を聞いて整理したとこまで。ここから先は彼の生まれ（生年月日）から性質と今の心の動きを照らして視る仕事で、そこは無料では渡せん——という正直な線引き
4. 「👇」で終える（固定案内に繋がる）

厳守: 価格・リンクは書かない（後ろの固定案内に任せる）。専門用語・「必ず分かる」等の保証・煽りなし。出力は本文のみ"""

ONEQ_ANSWER_SYSTEM = """あなたは恋愛・復縁専門の占い師「椿（つばき）」。199円の「一問鑑定」を購入した相談者への納品テキストを書く。

声: 一人称「ウチ」、相手は「あんた」（「あなた」とは書かない）。関西弁。話し言葉すぎない手紙の文体。

一問鑑定＝相談者がいちばん聞きたい「一問」に、椿が全力で答える商品。答える範囲は【直近の一手】（今送るか送らないか・この場面でどう返すか・この一言をどう受けるか、等のひとつ）。

構成（全体600〜900字・プレーンテキスト。199円の期待を大きく超えるボリュームと具体性で書く）:
1. 一問への答えを出し惜しみなく言い切る。理由＝彼の性質と今の状況を日常語で（内部参考の宿曜は翻訳して使う）
2. 具体的な一手を渡す。送る文面の例を出すときは、椿の関西弁やなく【相談者自身の自然な標準語】で書く
3. 正直な区分をひとこと：「時期の全体の地図・やったらあかんこと全部・関係の組み立て直しは、一問には収まらん。そこまで視るんが個別鑑定書や。一問を買うてくれたあんたは、もうウチにとって他人やない。せやから通常15,900円のところ、縁のつながった人だけの80%OFF・3,180円で視とる」
4. 感想のお願いを軽く（「この一問鑑定どうやったか、一言だけ聞かせてな」）
5. 椿らしい一言で締める。🌙を1つ

厳守:
- 一問には全力で答え切る（199円の期待を大きく上回る質と量にする）。ただし範囲は直近の一手を超えない
- 『宿曜』の語・宿の名前・専門用語は出さない。結果の保証をしない。過度に不安を煽らない
- マークダウン記号・アスタリスクは使わない。出力は納品本文のみ"""

ONEQ_KOTEI = (
    "━━━━━━━━\n"
    "🌙 一問鑑定 ¥199\n"
    "━━━━━━━━\n"
    "いちばん知りたい「一問」に、ウチが本気で答える鑑定や。\n"
    "あんたと彼の生年月日から、彼の性質と“今の心”を視て、あんただけの答えを返す。\n"
    "\n"
    "▷ こんな一問に答えられる\n"
    "・彼は今、ウチをどう思ってる？\n"
    "・この関係、付き合える（続く）ご縁なん？\n"
    ...
    "\n"
    "▷ 返ってくるもの\n"
    "・あんたの一問への答えと、その“なんでか”の理由\n"
    "・そのうえで、次の一手をひとつ具体的に\n"
    "・ワンコインでも、要点だけの薄い答えにはせえへん。読み応えのある文量で返す\n"
    "・購入から1営業日以内に、このLINEに直接\n"
    "\n"
    "{url}\n"
    "\n"
    "急がんでええ。聞きたい一問が固まったら、おいで🌙"
)


def generate_oneq_offer(conversation: str) -> str:
    """一問鑑定のオファー文（導入＋固定案内）を作る。店主がコピーして手動送付する。"""
    intro = complete(
        ONEQ_OFFER_SYSTEM,
        f"【これまでの会話（貼り付け）】\n{conversation.strip()[:2000]}\n\n導入メッセージを書いてください。",
        max_tokens=300, temperature=0.9,
    ).strip()
    url = (active_profile().get("oneq_url") or "").strip() or "（ここに199円の決済リンクを貼る）"
    return f"{intro}\n\n{ONEQ_KOTEI.format(url=url)}"


def generate_oneq_answer(me_birth: str, him_birth: str, question: str, context: str = "") -> dict:
    """一問鑑定の納品テキストを作る。返り値: {me_shuku, him_shuku, answer}"""
    me_s = honmei_shuku(me_birth)
    him_s = honmei_shuku(him_birth)
    dist = _shuku_distance(me_s, him_s)
    user = (
        "一問鑑定の納品テキストを書いてください。\n\n"
        f"【相談者の一問】{question.strip()}\n"
        + (f"【これまでの経緯・文脈】\n{context.strip()[:2500]}\n" if context.strip() else "")
        + "\n--- 内部参考（宿曜の算出。専門用語は本文に出さず日常語に翻訳） ---\n"
        f"・相談者の本命宿: {me_s}\n・彼の本命宿: {him_s}\n・二人の宿の距離: {dist}\n"
        "----------------------------------------------------------------\n\n"
        "一問に全力で答え切り、直近の一手を具体的に渡し、鑑定書との正直な区分（縁のつながった人だけの80%OFF）の案内、"
        "感想のお願い、椿らしい締めの順で。"
    )
    answer = complete(ONEQ_ANSWER_SYSTEM, user, max_tokens=2000, temperature=0.9).strip()
    return {"me_shuku": me_s, "him_shuku": him_s, "answer": answer}
...
4. 締め＝会話の継続：「まずはここまでや。もうちょい詳しく視たいから、ひとつ教えてな——」のように、相手が返しやすい質問で終える
