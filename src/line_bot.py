...
URL_NOTE = "https://note.com/tsubaki_honne/n/nbc7be8398a19"

# ★2026-08-13：一本売りから三択（個別鑑定書／潮見／構え）に組み替えた。
#   ここは実測10%で成約しとる唯一の装置やから、変える時の作法を書いておく。
#   ・「通常9,980円」の値引き表示は外した。潮見の9,800円と数字が近すぎて誤読するからや
#   ・値段の順に上から並べる。ほんで最後に「迷たら真ん中でええ」を必ず置く。
#     三択は迷いを生む。迷いを消す一行が無い三択は、一本売りより売れんくなる
#   ・「視る深さはどれも同じ」を頭に置く。いちばん下を選ぶ子を安う扱わんためや。
#     ここが抜けると、下を選んだ子が「安い方の客」にされた気分になる
#   ・リンクは三本になったが、動作は一つのまま（選んで踏む）。
#     「まずnoteを見て、それから買う」みたいな手続きは足さん。実データで場を移した回はリード0や
#   ★総成約率が7%を切ったら一本売りに戻すこと。三択は単価を上げるための賭けであって、
#     成約率まで上がる保証は無い。どの段が売れたかを毎週数える（週次KPIの3択ミックス）
OFFER_MENU = (
    "あんた専用に、彼の本音から、いつどう動くかまで視る。\n"
    "形が三つある。視る深さはどれも同じや。\n"
    "違うのは、読んだあとにあんたが迷うかどうかやねん。\n"
    "\n"
    "──────────\n"
    "　個別鑑定書　3,980円\n"
    "──────────\n"
    "彼の芯と二人の縁、彼の今の本音、次の一手まで。全8章・約10,000字や。\n"
    f"{URL_KANTEI}\n"
    "\n"
    "──────────\n"
    "　潮見　9,800円\n"
    "──────────\n"
    "鑑定書に、あんた専用の九十日の暦をつける。\n"
    "動いてええ日と、手を出さん日を、日付で出す。もう迷わん用や。\n"
    f"{URL_SHIOMI}\n"
    "\n"
    "──────────\n"
    "　潮見・構え　16,800円\n"
    "──────────\n"
    "潮見に、盤面が動いた時に切れる視直しの札を二枚と、\n"
    "そのまま送れる文面の札をつける。動いた後まで見る用や。\n"
    f"{URL_KAMAE}\n"
    "\n"
    "迷たら、真ん中の潮見でええ。待つ時間が長い子ほど、あれが効く。\n"
    "払い方はカードでもPayPayでもコンビニでもええ。\n"
    "\n"
...
def detect_signal(text: str, history: list[dict] | None = None) -> str | None:
    ...
    if history and _canned_ask_deeper_just_sent(history) and len(text) <= 200:
        if _REFUSAL_NEGATED_RE.search(text) or not _REFUSAL_RE.search(text):
            return "purchase"
    return None
