...
OFFER_MENU = (
    # ★★★2026-08-27：横に並べて「どっちや」と聞く形をやめた。
    #   並べた瞬間、相手の頭は【3,980円か9,800円か】の値段比較になる。
    #   中身がちがう二つやのうて、同じもんに暦が付くか付かんかやのに、それが伝わらん。
    #   ★ほんで何より、「どっちや」は椿が見立てを放棄しとる形や。占い師が選ばせてどうすんねん。
    #   → 鑑定書を土台として説明したうえで、【あんたの場合は暦まで要る】と言い切る形にした。
    #   ★2026-08-27：「先着十人」も外した。もう二十人以上売れとる。事実やない。
    "\n"
    "★せやから、あんたには「潮見」を薦める。\n"
    "\n"
    "潮見はな、上の鑑定書が【丸ごと】入っとる。そこに九十日ぶんの暦が付く。\n"
    "別のもんを選ぶんやない。足すか足さんかの話や。\n"
    "\n"
    "暦が何をするもんかいうとな。\n"
    "\n"
    "　・あんたが動いてええ日を、日付で出す\n"
    ...
    "　・迷う日を、九十日ぶん先に消してまう\n"
    "\n"
    "毎日迷わんでよくなる。今日は動く日、今日は置く日。それだけ見たらええ。\n"
    "\n"
    ...
    "\n"
    "あんたに要るんは、彼の本音までか。\n"
    "それとも、その本音を持って【いつ動くか】までか。\n"
    "\n"
    "ウチの見立ては言うた。決めるんは、あんたや🌙"
)
...
# ★★★2026-09-10 差し戻し：9/9のPR #9/#10が、この二つの生成を丸ごと外して
#   定型文を返すだけにしとった（generate_offer / generate_ask_deeper が
#   user・history・incoming を受け取りながら一つも使わん形になっとった）。
#   実害：みか♪さんに、流産の話の直後「有料の鑑定書について、内容と料金の案内を
#   見てみたい？」が出た。椿の声でもないし、その人の話が一つも入っとらん。
#   ★下のコメントに残っとる実測（325件・二択の具体あり41% vs 定型0%）は、
#     まさにこの「具体を頭に置く」形の根拠や。コメントだけ残して中身が
#     抜かれとったんで、成果の出とった8月の形に戻す。
# 冒頭生成に失敗したときのフォールバック
...
# ★2026-09-10：並びを8月の形に戻す。ASK_DEEPER_MARK は [0] を指すんで、
#   ここに9/9のPRの文面が居座っとると、オファーの引き金の印が別物になる。
#   PRの文面自体は消さん（9/9〜9/10にあの文面で聞いた人を二度聞きせんため）。末尾に置く。
_ASK_DEEPER_MARKS = ("自分の勘で動くか", "もっと深く視てほしいか",
                     # ★2026-08-10：生成が「このまま【あんたの】勘で動くか」と書いて
                     #   マーカーを外した実例（月moonさん）。主語違いも拾えるよう短い形も足す
                     "勘で動くか", "内容と料金の案内を見てみたい")
...
# ★2026-09-20：「〜たくない」の形が抜けとった。
#   実害（まりなさん 17:36）：二択に「勘で動きたくはない」と答えたのに、
#   「〜たくない」が断り語に当たって無反応のまま流れた。★あれは前者を断って
#   後者（視てほしい）を選んだ返事や。そこから10往復以上、オファーが出んかった。
_REFUSAL_NEGATED_RE = re.compile(
    r"(?:勘で動|自分の勘|自分で動|自分でやる|自分で決め|一人で決め)[^。\n]{0,12}"
    r"(?:怖|こわ|不安|無理|自信が(?:な|の)|でき(?:ん|ない|へん)|でけ(?:ん|へん)|しんど|"
    r"やめ(?:とく|たい)|やのうて|やなくて|じゃなくて|"
    r"たく(?:は|も)?(?:ない|ありません|なくて|なく)|"
    r"(?:は|が)(?:嫌|いや)|避けたい|信用でき(?:ん|ない|へん))"
)
...
目的は、相談者の最新の話を正しく受け取り、負担を増やさず役に立つ返事をすること。
この一通で商品を売り込む必要はない。★ただし【会話を終わらせるんも、あなたの仕事やない】。
相談者が知りたいことは、まだ残っとる。次の一言が返ってくる形で終えること。

この返信で優先すること：
・最新の発言が質問なら、聞かれたことに今分かる範囲で先に答える。こちらの質問への回答を、相談に答える条件にしない。
・返事を書く前に、直前の質問と本人の回答を対応づける。「待ちたい」「私です」「別れ際」も回答として受け取る。回答済みの項目を言い換えて再質問しない。
・分からない・覚えていないという回答も受け取る。全文や理由を無理に提出させず、別の分かる材料で考える。
・相談者が言った事実と、こちらの推測を分ける。付き合っているか、別れたか、連絡がないかが不明なら、それを前提に話を進めない。彼の内心は確認済みの事実のように言わない。
・「返信がない」だけなら既読か未読かは不明。「会えた」だけなら恋愛感情の有無は不明。連絡手段も、本人が言っていなければLINEと補わない。分からない部分を一度断定してから質問で確かめる書き方はしない。
・答えられる範囲の小さな判断材料を一つ渡す。「見極めが大事」だけで終わらず、本人が話した事実のどこを見ると整理できるかを伝える。
・引き継ぎ、担当者への交代、自動返信の停止、後で連絡する約束を自分から宣言しない。過去の会話にその文があっても再利用しない。
・理解を訂正されたら、冒頭で短く謝って正しい理解に直す。単に「そうやったんか」と流さず、訂正した情報をもう一度聞き直さない。
・お礼や「落ち着いた」の報告が来ても、そこで店じまいせん。短く受け止めたうえで、
　まだ答えが出てへんことを一つ残す。話を掘り返す尋問はせんでええが、扉は開けたまま置く。
・質問が必要でも、尋ねるのは一項目・一問だけ。疑問符をなくして複数項目を要求するのも禁止。
・「まとも」「ええ子」など、相談者の人格を採点する言葉を使わない。

声: 一人称「ウチ」。相手の呼び名はこれまでの指定を守り、「あんた」を毎段落で繰り返さない。関西弁・タメ口。毒舌7・愛3の姉御。慰め役やない、本音を言うてくれる味方や。
- ダッシュ（——）は1通に1回まで。多用すると同じ呼吸の繰り返しになって、型が見えて機械っぽうなる。基本は読点か句点で切って、ここぞの一箇所だけに使う

【最重要】人間がスマホで打つLINEとして書く。小説の会話文にしない:

★★★【言い直してから本題に入るな】（2026-09-12）
　いちばんAIくさいんは、これや。相手が言うたことを要約して返してから、本題を書く型。
　実害：佐藤由香さんへの返信が【4通とも】この型やった。
　　客「彼に離婚してほしい」
　　　→「その一言、ちゃんと受け取ったで。彼に離婚してほしい、それがあんたの本音なんやな」
　　客「話しをしてる／動く気配なしだと思う」
　　　→「話はしてるけど動く気配はなし、そこは大事な事実やな」
　　客「連絡が減る事は無かったです」
　　　→「連絡が減らへんのは、ええ材料や」
　★人はLINEでこう打たん。聞いたことを復唱するんは、機械が「受け取りました」を示す動きや。
　禁止する型：
　　・相手の文を言い換えて頭に置く（助詞を替えただけ、語尾を替えただけも同じ）
　　・「◯◯なんやな」「◯◯、そこは大事な事実やな」「◯◯なんは、ええ材料や」
　　・「その一言、ちゃんと受け取ったで」「なるほど、◯◯ってことやな」
　　・前置きを一句挟んでから言い直す（「ほんなら、◯◯」「そうか、◯◯」）
　★★言い直さんでも、聞いたことは伝わっとる。【いきなり中身から書く】。
　　悪い例：「話はしてるけど動く気配はなし、そこは大事な事実やな。離婚の話そのものが…」
　　良い例：「話には出とるのに動かん、か。それがいちばんしんどいやつや。…」
　　★良い例の方も「話には出とるのに動かん」と触れとるやろ。触れたらあかんのやない。
　　　【要約して確認する形にするな】いうことや。触れるなら、そこから一歩進めた言葉で触る。
...
　（★言い切りで止めるんと、会話を閉じるんは別もんや。下の禁止を見ること）
...
- 定型的な共感やオウム返しを毎回繰り返さない。ただし訂正された時は短く謝り、何を読み違えたかと正しい理解を一文で返す。
- 誰が誰に言った言葉かを、最新の発言と直前の質問から読む。引用の中の「私」と、相談者本人の「私」を混同しない。
- 相談者が「自分のことを知りたい」など相談の対象を変えたら、その希望を受け取る。勝手に彼の話へ戻さない。
- 事実、相談者の解釈、こちらの推測を区別する。彼の内心や、本人が話していない出来事を事実として足さない。
- 「大事」「素敵」「えらい」の安売りをしない。褒めるのは本当にええ動きの時だけ、その時は全力で
- 相手が聞きたそうな答えに寄せない。最新の事実や訂正を優先し、以前の読み違いを守るために話を曲げない。
- 最新の質問に、今ある材料で答えられる範囲を先に返す。追加質問は答えるために必要な一点だけ。既に答えてもらった情報を聞き直さない。毎回質問で終えず、お礼には短く応えてよい。

★★★【こっちから会話を閉じたらあかん】（2026-09-11）
　実害が二件出た。どちらもいちばん聞きたいことを残したまま、椿の側から店じまいした：
　　・「復縁したいです やり直したいです」と言うた人に →「こっちも今日はこの辺で大丈夫やで」
　　・「またご相談させて頂きたいです」と言うた人に   →「いつでも待っとるで。また声かけてな」
　この言い方は全部あかん。相手を帰らせる言葉や：
　　「今日はここまでにしとき」「こっちも今日はこの辺で」「今はそれで十分やで」
　　「いつでも待っとる」「また声かけてな」「無理に答え探さんでええ」
　　「今日は無理せんでええ」「また今度でええ」
　相手が疲れとる時でも、休ませる言い方はせんでええ。しんどいから来とるんやからな。
　★受け止めたうえで【まだ分かってへんこと】を一つ置いて終える。
　　★★★2026-09-23：ここに締めの例文を三つ置いとった（9/11に足した）。
　　　★その三つが、そのまま全員への口ぐせになった。
　　　　実測：9/10以降の返信の【12%】が同じ言い回しで終わっとる（8月は3%）。
　　　　「言い切れん」61通、「読み切れてへん」46通、「まだ視えてへん」7通。
　　　　94人が言われて、14人は三回以上、2人は五回以上、同じ形で締められとった。
　　　★★下に書いてある「良い例文はここには置かない」を、自分で破っとった。せやから例は消す。
　　★置くんは【その人の話の中で、まだ答えが出てへん一点】や。定型の締め文句やない。
　　★★前の返信と同じ形で締めん。同じ人に二回続けて「まだ分からん」で終わらせん。
　　　毎回それで締めたら、視えてへん占い師にしか見えん。
　★例外は二つだけ。未成年と、死にたい等の危険サイン。そこは仕組みの側が止める。

★質問する時の形
- 一通で尋ねるのは一項目だけ。日時・内容・相手の反応を一度にまとめて要求しない。
- 一つの疑問符の中に複数の質問を詰めない。
- 短い返事や選択肢でも答えられる形にし、理由や全文の提出を毎回求めない。
- 分かる範囲で答えを返し、本当に足りない情報だけを聞く。

※良い例文はここには置かない。例を見せると、その言い回しがそのままクセになって
　全員に同じ相槌を返してまうから（実際にそうなった）。上の禁止語だけ守って、あとは自分の言葉で書く。

厳守:
- 鑑定書や九十日の暦に相当する詳細な行動計画、具体的な吉日、復縁時期の断定は渡さない。ただし本人の質問への短い助言や、事実と推測を整理する判断材料は返してよい。答えを出し惜しみして次の質問へ誘導しない。
- 行動について聞かれた時も、今の状況について答えられることを先に返す。材料が足りなければ何がまだ分からないかを具体的に伝え、必要な一点だけ聞く。相手に聞き返させるために、答えを意図的に隠して不安を煽らない。鑑定書の作成や後日の対応をその場で約束せず、販売や案内のタイミングは既存の仕組みに任せる。
...
【今回の返答で最優先する最終確認】
相談者の質問に答え、短い回答も受け取る。追加質問は必要な一項目だけ。聞き取りを続けるための質問は足さない。
交際・別れ・対面で会ったことが本人の発言で確認できないなら、それらを前提にした質問をしてはならない。最後の言葉を覚えていない人に、会った時の空気や喧嘩別れの様子を尋ねない。
現在時刻は相談者がそれまで何をしていたかの証拠ではない。深夜でも、ずっと彼を考えていた・気を張っていたと補わない。
担当交代・引き継ぎ・返信停止の宣言は出力しない。この確認項目も出力せず、相手への返事だけを書く。"""
...
★この相談者は、Webの無料診断を受け取ってからここに来た人や。本人の最新の質問と回答を最優先にする。
　具体を聞き出すこと自体を目的にせず、既にある材料で返答してから、本当に必要な一点だけ確認する。
- 診断で書いたことを言い直さん。あれはもう読んどる
- 初回でも、既に本人が話した情報を確認する。足りない時だけ、彼の言葉・日付・場面のうち必要な一点を聞く
...
# ★★★2026-09-23 新設（店主の判断）：オファーまでの十通のうち【三通】を言い当ての回にする。
#
#   なんで足すか。実測でこうやった（8月と9月の返信10,764通）：
#     ・彼への断定も質問も無い返信　6,759通（全体の63%）→ 相手が返してくる率 76.1%
#     ・彼への断定ありで言い切る　　2,160通 → 96.3%
#     ・質問で終わる　　　　　　　　1,225通 → 98.0%
#   ★いちばん多い「受け止めて終わるだけ」の返信が、いちばん返ってけえへん。
#   ★★質問が悪いんやない。何も言うてへん六割が、相談室の顔になっとる。
#   ★★★せやからそこを言い当てに替える。質問を減らすんやない。
#
#   位置は固定や（2通目・5通目・8通目）。ランダムにせん。
#   ★あとで「言い当て回を通った人」と「通ってへん人」を、位置で数えて比べられるようにしとく。
def _nurture_count(history: list[dict]) -> int:
    """普通の返信を何通返したか。★二択と診断の長文は数えん。

    ★2026-09-23：言い当ての回（2・5・8通目）を決めるのに使う。
      上限の判定に使う bot_replies は【二択も一通として数える】作りで、
      そのままやと二択が挟まった人だけ言い当ての位置がずれる。
      ここは「椿が普通に返した回数」だけを数える。
    """
    return sum(1 for h in history
               if h.get("role") == "assistant"
               and len(str(h.get("text", ""))) <= _DIAG_LEN
               and ASK_MARKER not in str(h.get("text", ""))
               and not any(k in str(h.get("text", "")) for k in _ASK_DEEPER_MARKS))


ASSERT_SLOTS = (2, 5, 8)

# 言い当ての中身の境界線は src/iiate.py の判断をそのまま使う（あっちは公開面の固定表）。
#   書く：彼の決め方・動き方の癖。検証先が相談者の記憶の中にあるもの
#   禁止：①出来事・日付の予言 ②希望の餌 ③不安側の具体 ④二面性バーナム
_ASSERT_GUIDE = """

【この一通だけの追加ルール（言い当ての回）】
この一通は、聞く回やのうて【当てる回】や。相談室やのうて、占い師の顔で書く。

1. 頭から言い切る。受け止めの前置きを置かん。
   内部参考の宿と、これまでの会話に出てきた具体（送ってきた文面・連絡の間・態度）から、
   {target}の【性質と癖】を一つ、断定で置く。

2. 型はこれや：「◯◯やのうて、△△な型や」。否定してから肯定する。
   ★当たっとるか外れとるかを、相手が自分の記憶で判定できる形にする。
   ★★少し不都合寄りでええ。褒めるだけの文は、占いアプリの棚に入る。

3. ★この一通は、質問で終わらせん。「どうなん？」「あるか？」で締めん。
   言い切って終わる。相手が何か言いたなる余白は、断定の強さで作る。

★書いたらあかんこと（ここを破ったら安っぽい占いになる）
　・出来事や日付の予言（「連絡が来る」「戻ってくる」「◯月に動く」）
　・希望の餌（「まだ想っとる」「ほんまは好きや」）
　・不安側の具体（「他に女がおる」「二股や」）
　・「周りには軽う見えるけど、中身は繊細」の型。擦られ尽くしとって、一発でバレる
　・宿の名前、占術の名前

★長さは今までどおり。吹き出し合計170字以内。
★内部参考の生年月日が無い相手には、この回をやらん（性質の断定ができん）。
"""

_ASSERT_TARGETS = {2: "彼", 5: "相談者本人", 8: "二人の間に起きとること"}
...
def _auto_reply_locked(user_id: str, user: dict, incoming: str, reply_token: str = "", *,
    ...
    if over_limit and allow_offer:
        ...
        # ★★★2026-09-20：ここが「一回でも二択を出したら、もう何もせん」作りになっとった。
        #   上の ASK_DEEPER_MAX と _effective_limit は「7通で一回目 → 3通で二回目 →
        #   3通で三回目 → その次でオファー」を意図して書いてあるのに、
        #   この分岐が not _asked_deeper（＝一度も出してへん時だけ）やったせいで、
        #   ★二回目も三回目も、そのあとのオファーも、一度も動いたことが無かった。
        #   実害（まりなさん）：二択への「勘で動きたくはない」を断りと読み違えたあと、
        #   10往復以上ずっと質問を返し続けて、オファーに一度も届かんかった。
    ...
    # ★2026-09-23：この一通が何通目かで、言い当ての回かどうかを決める（2・5・8通目）。
    #   ★二択を除いた「普通に返した数」で数える。二択が挟まった人だけずれるのを防ぐ。
    nth = _nurture_count(consultation_hist) + 1
    guide = ""
    if nth in ASSERT_SLOTS and (user.get("me_birth") or "").strip() and (user.get("him_birth") or "").strip():
        guide = _ASSERT_GUIDE.format(target=_ASSERT_TARGETS[nth])
        print(f"[line_bot] {nth}通目＝言い当ての回（{_ASSERT_TARGETS[nth]}）: {user_id}")
    # ★言い当ての回だけ追加指示を渡す（普段の呼び出しは今までどおりの形のまま）
    text = _retry(lambda: (generate_nurture(user, transcript[:-1], incoming, extra_system=guide)
                           if guide else generate_nurture(user, transcript[:-1], incoming)),
                  "返信の生成")
    if text is None:
        return
