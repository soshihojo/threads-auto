...
# ★2026-08-14：商品ごとの個別URLをやめて、店のトップ一本にした。
#   三つ並べると本文にリンクが三本立って、どれを踏むかで一手増える。
#   店のトップやと三つが一枚に並んどるので、見比べてから選べる。
#   個別ページのURLも残しとく（一人一商品を名指しで勧める時に使う）。
...
OFFER_MENU = (
    ...
    "\n"
    # ★2026-08-14：商品ごとの説明を一行ずつに削り、リンクは店のトップ一本だけにした。
    #   前は商品ごとにURLが三本立って、本文が長うなるうえ「どれを踏むか」で一手増えとった。
    #   値段と一行の中身だけ見せて、比べるんは店の一枚でやってもらう。
    #   ※noteの見本はこの便から外して、直後の二通目に回した（リンクは一通に一本）。
    "──────────\n"
    "　個別鑑定書　7,980円 → 3,980円\n"
    "──────────\n"
    "彼の本音と、次の一手まで。全8章・約10,000字。\n"
    # 数量限定（先着10人）。実測の購入ペースは1日2.3件なので、10枠は4〜5日で埋まる。
    # 書いた以上は10人で戻すこと。戻さんまま出し続けたら、
    # 7/26に信用を削った「数量限定」の再演になる。
    "この一番下だけ、今は先着十人までこの値や。\n"
    "\n"
    "──────────\n"
    "　潮見　9,800円\n"
    "──────────\n"
    "鑑定書に、あんた専用の九十日の暦をつける。\n"
    "動いてええ日と、手を出さん日を、日付で出す。\n"
    "\n"
    "──────────\n"
    "　潮見・構え　16,800円\n"
    "──────────\n"
    "潮見に、視直しの札を二枚と、そのまま送れる文面の札をつける。\n"
    "\n"
    "三つとも、この一枚に並べてある。見比べてから選び。\n"
    f"{URL_STORE}\n"
    "\n"
...
#   一通目＝買う場所（店のトップ）、この二通目＝中身の見本と受けた子らの声（note）。
