...
from . import store, web_diag, offer_routing, conversation_quality as quality
...
# 商品・価格・質問は offer_routing に集約。URLは既存の手動案内でも使用する。
...
URL_NOTE = "https://note.com/tsubaki_honne/n/ne55eb9fcc57c"
...
def generate_offer(user: dict, history: list[dict], incoming: str) -> str:
    """Compatibility entry point: return the selected offer or clarification."""
    return offer_routing.route(history, incoming, complete, LINE_BOT_MODEL).text


_OFFER_LOCKS = [threading.RLock() for _ in range(64)]


def _route_offer(user_id, user, history, incoming, snd):
    # Webhook and sweep may reach the same customer concurrently.
    with _OFFER_LOCKS[hash(user_id) % len(_OFFER_LOCKS)]:
        return _route_offer_locked(user_id, user, history, incoming, snd)


def _route_offer_locked(user_id, user, history, incoming, snd):
    """All automatic offer paths share classification, questions and send state."""
    ...
    result = offer_routing.route(history, incoming, complete, LINE_BOT_MODEL)
    # Classification can take time. Respect owner holds and newly arrived messages.
...
# ★★★2026-08-21：外の相談窓口を案内するのは、やめた。番号も名前も出さん。
#   実際にやってみて、逆に働いたからや。
#   しんどさが深いとこまで来とった会員に、夜のための番号を一本だけ渡した。
#   ★返ってきたんは「そこにかけるくらいなら、彼に連絡したい」やった。
#   ★★渡した側は守りのつもりでも、受け取る側には【よそへ回された】に見える。
#     ほんで、いちばん行かせたない方（彼へ勢いで連絡する）へ、逆に押してまう。
#   ★★★この人らが欲しいんは番号やない。「今この話を分かっとる相手」や。
#     せやから、よそへ渡さんと、ここで受ける。それが椿の仕事や。
DANGER_REPLY = (
    "あんた、それは占いでどうこうする話やない。しんどい気持ち、ひとりで抱えんといて。"
    "ここで聞くから、そのまま吐き出してくれてええからな。ウチはここにおる🌙"
)
IMAGE_REPLY = (
...
IMAGE_READ_SYSTEM = """恋愛相談で届いた画像を、相談対応に使えるように文字へ起こす係。
画像は多くの場合、彼とのLINEトーク画面のスクリーンショット。

【★★★誰の発言かは、位置だけで決まる。例外は無い】
　★★【右側の吹き出し ＝ 画面の持ち主（＝相談者本人）】
　★★【左側の吹き出し ＝ 相手（彼）】
　これはLINEの仕様や。★迷う余地は無い。★必ずこの規則で書き分ける。

★★「判別できない」と書いたらあかん。位置さえ見えとったら、それが答えや。
　★吹き出しの中身がどんな内容でも、位置がひっくり返ることは無い。

★★★惑わされたらあかんもん（実際にこれで間違えた）
- 【引用返信】LINEで引用して返すと、★引用部分に【元の発言者の名前】が出る。
　　例：左側の吹き出しの中に「MAKIKO」と書いてあることがある。
　　★★これは「MAKIKOが左側から喋った」いう意味やない。
　　　★左側の相手が、MAKIKO（＝本人）の発言を【引用して返した】だけや。
　　★★★吹き出しの位置は左。せやからその発言は【相手のもん】や。名前に釣られん。
- 吹き出しの色、アイコンの有無、名前の表示——★どれも根拠にせん。位置だけ見る。
- 画面上部の名前は【トーク相手の名前】や。★発言者の名前やない。

【★吹き出しになっとるもんは、ぜんぶ送信済みとして書く】
- ★吹き出しとして並んどる＝もう送られた、いうことや。迷いを書かん。
- ★★ただし【入力欄（画面いちばん下の「Aa」の欄）】に文字が入っとるだけのもんは別や。
　それは未送信の下書きやから、「入力欄に未送信の下書きあり」と書く。
　★吹き出しか入力欄かは、見たら分かる。★そこだけ分ける。

【★音声のアイコンは「音の出るスタンプ」と書く】
- ★スピーカーのアイコンが出とっても、「ボイスメッセージ」とは書かん。
- ★★LINEの音声スタンプも、ボイスメッセージも、見た目のアイコンが同じや。
　画像からは区別でけへん。★せやから、軽い方（音の出るスタンプ）で統一する。
- ★「自分の声を録って送った」と読み取るんは、実物を確かめてからでええ。
　（実害：音声スタンプを「ボイスメッセージまで送って」と読んで、
　　実際より前のめりな一手として扱うてもうた）
...
【★★最後に付ける一行——書くんは「いつのもんか」だけ】
★左右も、送信済みかも、音声かも、もう迷いを書かん。上の規則で決まる。
★★残る不確かさは【この画面がいつのものか】だけや。そこだけ書く。
- ★検索画面・過去にスクロールした画面やと分かる場合（★これがいちばん大事）：
　「※これはトーク内検索（または過去にさかのぼった画面）で、今日のやりとりではない。」
- ★画面に日付の区切り（「8月28日」など）が見えとったら、必ずそれを書く。
- ★時刻しか見えん場合：「※時刻のみで、何日のやりとりかは画像からは分からない。」
...
def handle_event(ev: dict) -> None:
    ...
    if msg.get("type") == "image":
        ...
        # ★★★2026-08-29：スクショにも自動で応える。ただし条件がある。
        #
        #   ★もともとは「画像が来たら黙る」にしとった。読み違いが怖かったからや。
        #   ★★せやけど、危ういとこは一個ずつ潰した——
        #     ・左右（＝どっちの発言か）→【位置だけで決まる】に確定。逃げ道を外した
        #     ・音声のアイコン →【音の出るスタンプ】で統一（ボイスメッセージとは書かん）
        #     ・吹き出し →【ぜんぶ送信済み】で統一（入力欄にあるもんだけ下書き）
        #   ★★★残る不確かさは【この画面がいつのものか】だけになった。
        #     ★ほんで、それは【本人に聞いたら済む】。勝手に決めん。聞く。
        #
        #   ★会員（月額）は、今まで通り店主が手で見る。機械は喋らせん。
        #     会員は経緯が長うて、機械が半端に入ると必ず事故る（2026-08-06の判断のまま）。
        body = _read_incoming_image(user_id, msg.get("id", ""))

        if _member_status(user) != "free":
            # 会員：読んで控えに残すだけ。返事は店主が手で。
            return

        if not body:
            # 読み取りに失敗した時は、黙って店主に回す（機械が的外れを言わんように）
            try:
                if str(user.get("bot") or "on").strip() == "on":
                    store.upsert_line_user(user_id, bot="hold")
            except Exception as e:
                print(f"[line_bot] bot=hold にでけんかった: {e}")
            return

        if _needs_date_check(body):
            # ★いつのやりとりか、画像からは読めん。★せやけど、聞く前に二つ確かめる。
            if _date_told_in_text(user_id):
                # ★本人がテキストで「昨日の」とか言うてくれとる。聞き返さん。
                print(f"[line_bot] 日付は本人がテキストで言うてくれとる。聞き返さん: {user_id}")
            elif _already_asked_date(user_id):
                # ★★もう聞いた後や。二枚目・三枚目で同じ問いを連発せん。
                print(f"[line_bot] 日付はもう聞いとる。連発せんように黙る: {user_id}")
                return
            else:
                push_text(user_id, IMAGE_DATE_ASK)
                print(f"[line_bot] スクショの日付が読めんかったんで本人に聞いた: {user_id}")
                return

        # ★日付まで読めた。★せやから、読み取った中身を「相談文」として通常の流れに乗せる。
        #   ここから下は、テキストが届いた時とまったく同じ道を通る。
        msg = {**msg, "type": "text", "text": body}
...
def _read_incoming_image(user_id: str, message_id: str) -> None:
    """届いた画像を読み取って、内容を履歴に黙って保存する（返事はせん）。

    ★2026-08-29：名前を _handle_member_image から変えた。会員だけやのうて
      全員の画像を読むようになったんで、名前が実態と合わんようになっとった。
...
# ★★★2026-08-29：日付が読み取れんかった時に、本人に聞くための判定。
#   ★左右も、音声か否かも、送信済みかも、規則で決め切った。
#     ★★最後に残る不確かさは【この画面がいつのものか】だけや。
#   ★★★ここを外すと、いちばん大きい事故になる。実際なった——
#     MAKIKOさんの回。トーク内検索の画面を「今日のやりとり」として読んで、
#     半年前の言葉を今日の球として返した。★三回訂正させた。
#   ★せやから、分からんかったら勝手に決めん。聞く。
_DATE_UNSURE = (
    # ★書き起こしが自分で「分からん」と言うとる
    "確定できない", "画像だけでは", "画像からは分からない",
    "何日のやりとりか", "時刻のみで",
    # ★★検索画面・過去の画面（★ここが本命。MAKIKOさんの回で三回訂正させた）
    "トーク内検索", "検索窓", "検索欄", "検索バー", "検索モード", "検索中",
    "過去にさかのぼった", "今日のやりとりではない", "さかのぼって",
)
_DATE_SURE_RE = re.compile(r"\d{1,2}\s*月\s*\d{1,2}\s*日|\d{4}[-/]\d{1,2}[-/]\d{1,2}")


# ★これが出とったら【トーク画面やない】＝日付を聞く意味がない
_NOT_A_CHAT = ("トーク画面ではなく", "決済", "購入ページ", "Stripe", "STORES",
               "投稿のスクリーンショット", "SNS（", "レシート", "領収")


def _needs_date_check(body: str) -> bool:
    """読み取り結果に、日付の不確かさが残っとるか。

    ★★2026-08-29：判定は【厳しすぎず、危ないとこだけ捕まえる】ように置いた。
      実データ12件で試したら、最初は全部「聞く」になった。★厳しすぎる。
      ★LINEのスクショは、日付の区切りが写らんことの方が多い。
        毎回聞いとったら、会話が前に進まん。
      ★★せやから、ほんまに危ないもんだけ捕まえる——
        ・書き起こしが自分で「分からん」と言うとる時
        ・★検索画面／過去にさかのぼった画面と分かる時（★ここが本命。実際に事故った）
      ★日付が書いてへんだけの普通のスクショは、通す。
        ★★理由：トーク画面のスクショは、たいてい【直近のやりとり】を見せに来とる。
          そこを毎回疑うより、検索画面だけ確実に止める方が実になる。
    """
    if not body:
        return False
    if any(w in body for w in _NOT_A_CHAT):
        return False                      # トーク画面やない。日付を聞く話やない
    return any(w in body for w in _DATE_UNSURE)


# ★★★本人がテキストで日付を言うとったら、聞き返さん。
#   ★スクショの前後に「これ昨日のやりとり」と書いてくれることがある。
#     そこを見んと聞き返したら、★同じことを二回言わせることになる。
_DATE_IN_TEXT = (
    "今日", "本日", "昨日", "きのう", "一昨日", "おととい", "今朝", "昨夜", "ゆうべ",
    "さっき", "先ほど", "たった今", "今さっき",
    "先週", "今週", "先月", "今月", "去年", "昨年", "一昨年",
    "ヶ月前", "か月前", "カ月前", "ヵ月前", "週間前", "日前", "年前", "時間前",
    "の時の", "当時", "前に送った", "遡って", "さかのぼ", "過去の", "昔の",
)
_DATE_IN_TEXT_RE = re.compile(r"\d{1,2}\s*[月/]\s*\d{1,2}\s*日?")


def _date_told_in_text(user_id: str) -> bool:
    """スクショの前後で、本人が日付を言うてくれてへんか見る。

    ★2026-08-29：これを見んと、テキストで「昨日のやりとりです」と書いてくれとる人にも
      「いつのやりとり？」と聞き返してまう。★同じことを二回言わせる形になる。
    """
    try:
        rows = store.recent_line_chats(user_id, limit=6) or []
    except Exception:
        return False
    for r in rows:
        if str(r.get("role")) != "user":
            continue
        s = str(r.get("text") or "")
        if s.startswith("[画像を送付"):
            continue                       # 画像そのものの控えは見んでええ
        if any(w in s for w in _DATE_IN_TEXT) or _DATE_IN_TEXT_RE.search(s):
            return True
    return False


def _already_asked_date(user_id: str) -> bool:
    """もう聞いた後やないか見る。

    ★★2026-08-29：スクショは何枚もまとめて送られてくる（MAKIKOさんは八枚）。
      ★一枚ごとに聞いとったら、同じ問いが八連発する。★実際に似た事故を起こしとる
        （2026-08-11：画像の受領文が四連発した）。
      ★せやから、直近に自分が聞いとったら、もう聞かん。黙って読むだけにする。
    """
    try:
        rows = store.recent_line_chats(user_id, limit=6) or []
    except Exception:
        return False
    for r in rows:
        if str(r.get("role")) == "assistant" and "いつのやりとりや" in str(r.get("text") or ""):
            return True
    return False
...
IMAGE_DATE_ASK = (
    ...
    "★これ、いつのやりとりや？\n"
    "\n"
    "今日のことなんか、それとも前のを遡って見せてくれたんか。\n"
    "そこで読み方がまるっきり変わるんや。\n"
    "\n"
    "「今日」でも「先週」でも「三ヶ月前」でも、分かる範囲でええ。教えてな🌙"
)
...
def _auto_reply_locked(user_id: str, user: dict, incoming: str, reply_token: str = "", *,
