...
def generate_nurture(user: dict, history: list[dict], incoming: str) -> str:
    ...
    # ★2026-08-08（陽子さん）：「どっちや？」に「私」と完全な答えが来たのに、
    #   「『私』だけ来ても続きがわからんで」と聞き返した。
    #   短い返事は直前の質問への答えや。それを「分からん」と返したら、
    #   読んでへん人にしか見えん。検知したら作り直させる。
    if len(incoming.strip()) <= 8 and _REASK_RE.search(text):
