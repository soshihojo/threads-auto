...
_HEDGE_RE = re.compile(
    ...
    # ★2026-08-20：「ても／ては／まで／までは」を足した。
    #   「返事が来ても追わん」「既読が付くまで動かん」は、起きてから何をするかの話や。
    #   予言やのうて条件やから、ここで見逃す。
    r"|(?:ても|ては|まで|までは|"
...
# ★★★2026-08-20 追加。主語を省いた予言。
#   暦の行から「あんたは」の繰り返しを外した結果、この穴が実害になった。
#   _HIM_FUTURE_RE は「彼」「相手」「向こう」のどれかが要る作りなんで、
#   主語を落として「連絡が来る週や」と書かれたら、そのまま通ってまう。
#   ★暦は主語を省いてよい。せやけど省いてええんは【相談者の行動】だけや。
#     到来（来る・届く）を主語なしで書いたら、それは彼の未来を書いとるんと同じことになる。
#   ★★strict のときだけ見る。鑑定書の本文では「連絡が来るのを待つ」みたいな
#     ふつうの言い回しを使うんで、そっちまで止めたら書けんようになる。
_ARRIVAL_RE = re.compile(
    r"(?:連絡|返信|LINE|ライン|電話|音沙汰|返事|一言|反応|既読)"
    r"[^。！？]{0,10}?"
    r"(?:来る|くる|入る|届く|寄こされる|返ってくる|戻ってくる|付く|つく)"
)
...
def check_prophecy(text: str, strict: bool = False) -> list[Problem]:
    ...
    for lineno, line in _lines(text):
        for sent in _sentences(line):
            ...
            if not m and strict:
                m = _ARRIVAL_RE.search(sent)   # ★主語を省いた到来も、暦では予言や
            if not m:
                continue
