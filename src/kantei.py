...
SHUKU_TRAITS = {
    "氐宿": "面倒見がよく世話好きで、頼られると断れない。人当たりが柔らかく懐が深いが、自分のことは一切相談せん。抱えとる時ほど静かになり、内側で処理してから外に出す。情でつながる質で、理屈で切ったつもりでも情だけは残る。自分から踏み込むんは苦手で、動くのはたいてい相手からの働きかけがあった後。約束を軽い気持ちで口にするが、段取りを組む力は弱く、言うたことと実行が別々に立っとる。責任や型にはめられるのを嫌い、生活の領域に他人を入れることに強い抵抗がある",
...
- ダッシュ（——）は1章に1〜2回まで。多用すると同じ呼吸の繰り返しになって、読ませる文章が単調になる。基本は読点か句点で切る
...
- 【名乗らない】「はじめまして」「椿と申します」「占い師の椿です」の類を、絶対に書かない。
　★理由は二つ。①この相談者とは既にLINEで何往復もやりとりしとる。初対面やない。
...
　実害：「三歳で別れて二十歳まで会えていない」を「二十年会えんかった」と書いた（正しくは十七年）。
- 【本人が言うてない言葉を鉤括弧で引かない】相談者が実際に書いた言葉だけを引用する。
　こっちで作った台詞を「」で括ったら、本人には自分が言うてへんことがすぐ分かる。
　実害：「『確実なもんが欲しい』——あんたのその気持ち」と、言うてもない言葉を引いた。
- 【月額の案内を書かない】月額会員・継続相談の案内は、鑑定書の本文に一切書かん。
　感想が返ってきてから、別の文で案内する導線になっとる。
　とくに「無理に、とは言わん」のように、入会を見送らせる書き方は禁止や。
- 指定された章の内容だけを書く。他の章で扱う内容を先取りしない。章タイトルや見出しは書かず、本文だけを出力する\n- 【絶対厳禁】書き方の見本として渡される『別の相談者の一件』の事実を、この人の鑑定書に持ち込まない。相談者から届いた詳細に書かれていない出来事・台詞・年数・習慣は、どれだけ話が繋がって見えても一つも書かない。これは金を貰って渡す紙で、他人の人生を書いたら取り返しがつかない"""

# ★2026-08-15：例文の一件（怜と真翔）が、別の相談者の鑑定書に丸ごと流れ込む事故が出た。
#   あやかさんの鑑定書に「怜」が17回、「真翔」が2回。それだけやのうて中身まで入れ替わっとった。
#     ・彼が言うてもない台詞「既読無視ごめん。怜の気持ち知ってて…」が引用として本文に入った
#     ・「十年キープしとった」（あやかは六年）
#     ・第六章が丸ごと「会える友達に戻る」の話になっとった。
#       あやかは家を出て前に進む人で、真逆の指示を書いとった
#   なんで飲まれたか：あやかの一件が例文と構造的に似すぎとった。
#   彼が大型トラック運転手（例文はタンクローリー運転手）、長い友達期間から交際、
#   一線を越えた過去。似た形を見た瞬間、生成が例文の記憶で埋めにいった。
#   過去34件は無事やった（怜さん本人の分を除く）。似た人が来た時だけ起きる事故や。
#   ★教訓：見本を濃く書くほど品質は上がるが、似た人が来た時に飲まれる。
#     見本を薄めるんやのうて、出口で検査する。ここは金を貰うて渡す紙や。
# 見本の人の名前。相談者の詳細に出てこん限り、本文に出たら即アウト
_EXAMPLE_WORDS = ("怜", "真翔")
# 見本の一件にしか出えへん言葉。★相談者から届いた詳細に同じ言葉が無い時だけ弾く。
#   （例：ほんまに酒がらみの相談なら details に「酔って」が出るはずで、その時は通す）
_EXAMPLE_FACTS = (
    "既読無視ごめん", "期待には応えられない", "一方的でごめん", "タンクローリー", "いわき",
    "酔って", "酒の力", "酒の勢い", "素面", "押しかけ", "バイト先", "飲み会",
    "久しぶり！元気", "会える友達", "特に何もないのに", "十年",
    # ★★★2026-08-28：はるちゃんの回で、ここに無い形で漏れた。実際に出た語を足す。
    #   見本の怜さんは「酔った勢いの過去」「告白の記憶が無い」「二十歳から十年の友達」や。
    #   その骨格が、言い換えられて本文に入り込んだ。
    #   ★「十年」は網にあったのに「十五年」で抜けた。数字を変えられたら素通りしとった。
    "シラフ", "覚えとる自分", "何送ったっけ", "記憶の外", "覚えてへんこと",
    "十五年", "十年もっとる", "十年の友達", "二十歳で出会",
)

# ★★2026-08-28：見本は「酒で失敗した人」の話や。
#   相談者の詳細に酒の気配が【一つも無い】のに、本文だけに酒や記憶の話が出てきたら、
#   それは百パーセント見本からの漏れや。★ここは一語も通さん。
_EXAMPLE_DRINK_ONLY = ("シラフ", "覚えとる自分", "何送ったっけ", "記憶の外",
                       "酔って", "酒の力", "酒の勢い", "素面")
...
def _example_leaks(body: str, details: str) -> list[str]:
    ...
    for w in _EXAMPLE_FACTS:
        ...
        hits.append(w)
    # ★★2026-08-28：相談者の詳細に酒の気配が一つも無いのに、本文にだけ
    #   酒や記憶の話が出てきたら、それは見本からの漏れや。無条件で弾く。
    if not drinking:
        hits += [w for w in _EXAMPLE_DRINK_ONLY if w in body]
    # ★年数の水増しも見る。「十五年」「十年」は見本の相談者の話や。
    #   相談者の詳細に同じ年数が書いてへんかったら、それは作り話や。
    for w in ("十五年", "十四年", "十三年", "十二年", "十一年"):
        if w in body and w not in det:
            hits.append(w)
...
# ★2026-08-15：章タイトルが初回コミットから例文の名前（怜・真翔）のままやった。
#   過去の納品物は、店主が毎回HTMLを手で直して出しとったことになる。
#   相談者の名前と彼の呼び名で埋める形にして、手直しを要らんようにする。
def _him_name(details: str) -> str:
    """相談者から届いた詳細の【彼】行から呼び名を拾う。取れんかったら「彼」。"""
    m = re.search(r"【彼】\s*([^\n。、（(]{1,12})", details or "")
    nm = (m.group(1).strip() if m else "")
    # 「大輔（彼が麻見を…」のような書き方でも、頭の呼び名だけ取れるようにしとる
    return nm or "彼"
...
def generate_chapters(name: str, me_birth: str, him_birth: str, details: str,
    ...
    # ↓ 実物の事故は _EXAMPLE_LEAK_RE の定義（このファイルの上の方）を見よ
    today = today or datetime.now().strftime("%Y-%m-%d")
    brief = _internal_brief(name, me_birth, him_birth, today)
    him_name = _him_name(details)
    def _fill(t: str) -> str:
    ...
    for key, raw_title, chars, instruction in CHAPTERS:
        ...
        user = (
            ...
            f"=== 今回書く章 ===\n章タイトル: {title}\n目安の分量: {chars}字（±2割）\n\n"
            "=== この章の書き方（※重要な読み方の注意） ===\n"
            "以下の指示には、【別の相談者の一件】が具体例として書き込んである。"
            "これは狙い・深さ・語り口の見本であって、この人の事実やない。"
            "見本に出てくる人名・出来事・台詞・年数・習慣を、一つも本文に持ち込んだらあかん。\n"
            f"{instruction}\n\n"
            "=== 書くときの絶対の制約 ===\n"
            "本文は『相談者から届いた詳細』に書いてある事実だけで組むこと。"
            "そこに無い出来事・台詞・数字・癖・過去を、一つも作ったらあかん。"
            "見本と似た場面が思い浮かんでも、この人の詳細に無いなら書かん。\n\n"
            "本文だけを出力してください。"
        )
        ...
        for attempt in range(3):
            ...
            body = strip_jargon(strip_ai_leak(strip_markdown(complete(
                ...
                f"とくに次の語は一度も書いたらあかん: {'、'.join(_EXAMPLE_WORDS)}\n"
                "相談者から届いた詳細に書いてへん出来事・台詞・数字は、一つも作らんこと。",
                max_tokens=3000, temperature=0.6).strip())))
        else:
            if _example_leaks(body, details):
                raise RuntimeError(
                    f"「{title}」から例文の混入が消えん。この鑑定書は出せん。"
                    "入力（details）が例文の一件と似すぎとる可能性がある。"
                    "手で章を書くか、入力の書き方を変えてやり直すこと")
...
7. 感想を求めたあと、最後に一言だけ【線引き】を置く。
   ...
   ほんで最後に一個だけ言うとくな。
   読んだあとに出てくる「で、私はこれからどう動いたらええん？」——
   そこから先の細かい一手を受けとる場は、ちゃんと別に用意してある。
   そういう場があるいうことだけ、今のうちに置いとくわ🌙

   ★★2026-08-06：ここで「月詠み」という商品名を出したらあかん。
   相談者はその名前をまだ知らん。知らん名前を出された瞬間、
   案内やのうて「よそ行きの売り文句」に見えてまう。
   ★★★2026-08-17改定：商品名だけやのうて「別のプラン」もあかん。
   プランと呼んだ時点で、そこに商品があると名指ししとることになる。
   funnel ②-1 の指定どおり「場が別に用意してある」とだけ示唆する。
   名前と中身と値段を出すのは、感想が返ってきてからの便でええ。

厳守:
- 料金・決済リンクは書かない（感想が返ってきてから別便で送るため）。
  ただし上の7の四行は必ず入れる（商品名も「プラン」も出さず、場があるとだけ書く）
...
DELIVERY_CLOSING = (
    "ほんで最後に一個だけ言うとくな。\n"
    # ★★2026-08-20：ここの一人称は「私」や。「ウチ」は椿の一人称やから、
    #   相談者の心の声に使うたら、誰が喋っとるんか分からんようになる。
    "読んだあとに出てくる「で、私はこれからどう動いたらええん？」——\n"
    # ★★2026-08-20：「別のプラン」も商品を名指ししとることになるんで、「場」に戻した。
    #   funnel ②-1 の指定は「その先の一手を受けとる場は、ちゃんと別に用意してある」や。
    "そこから先の細かい一手を受けとる場は、ちゃんと別に用意してある。\n"
    "そういう場があるいうことだけ、今のうちに置いとくわ🌙"
)
...
def _fix_delivery_closing(text: str) -> str:
    ...
    body = body.replace("椿の月詠み", "別に用意してある場").replace("月詠み", "別に用意してある場")
    return f"{body}\n\n{DELIVERY_CLOSING}"
