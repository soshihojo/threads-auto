...
7. 感想を求めたあと、最後に一言だけ【線引き】を置く。
   鑑定書を読んだあとに出てくる「で、これどうしたらええ？」は月詠みで受ける、という案内。
   売り込みにせず、事実として軽く一行。押しつけがましくせず、
   「そこは最初に言うとくな」くらいの温度で置くこと。
   ※これは値段や決済リンクを出す意味ではない。存在と線を伝えるだけ

厳守:
- 料金・決済リンクは書かない（感想が返ってきてから別便で送るため）。
  ただし上の7で、月詠みの「存在と線引き」に一行だけ触れるのは必須
