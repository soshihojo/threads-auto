...
def _draft_reply(reply_text: str, username: str, is_lead: bool,
    ...
    # ★★★2026-09-07：誘導する回に「生年月日」が抜ける事故を止める。
    #
    #   実害：config の「手順の説明（生年月日の入れ方…）を並べない」が
    #   ★「生年月日は一語も書くな」と読まれた。
    #     9/1　返信の85%に「生年月日」あり → 返信143件 → ページ表示240件（168%）
    #     9/7　返信の 0%                  → 返信146件 → ページ表示 53件（ 36%）
    #   ★★「固定投稿の無料鑑定で数字が答えてくれる」——これでは何をするんか分からん。
    #     読む側は「何を書いて送ればええんやろ」で手が止まる。止まったら、もう動かん。
    #
    ...
    if is_lead and "固定投稿" in text and "生年月日" not in text:
        ...
        text = _clean_reply(complete(
            system + "\n\n【厳重注意】固定投稿へ誘導するなら、"
                     ...
                     "★★ただし手順として並べん。"
                     "「二人の生年月日から出てくるわ」のように、一文の中へ自然に流し込むこと。",
            user, model=REPLY_MODEL, max_tokens=200, temperature=0.9))
