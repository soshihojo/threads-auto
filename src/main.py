...
def _check_name(name: str) -> str:
    ...
    ★★★2026-09-13：店主の判断で【どんな時でも さん を付ける】に変えた。
      それまでは「渡された呼び名のとおり。足しも引きもせん」やったが、
      「ちー」「まや」で渡した潮見が本文で呼び捨てになった。

    ★★2026-08-30の経緯も残しとく（同じ穴を二度掘らんため）：
      ・「ゆみさん」で渡した → 暦の表紙が【「ゆみさんさんのために」】になった
      ・ほな剥がしたろ、と「ゆみ」に直した → 章タイトルが【「ゆみという人」】と呼び捨てに
      ★どっちも納品直前まで行った。★原因は「足す側」が素で さん を足しとったことや。
      せやから足す側（diagnosis.with_honorific）は今も敬称の有無を見る作りのままや。
      ★ここで さん を足しても、with_honorific が二重に足すことは無い。
    """
    n = str(name).strip()
    if n.endswith(("さん", "ちゃん", "くん", "様", "さま", "君")):
        return n
