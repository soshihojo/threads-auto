...
| ブロック | 機能 | 状態 |
|---|---|---|
| A. 投稿（承認モデル） | 6時間ごとに候補を自動生成→**ダッシュボードで承認**→予約時刻にrun-dueが配信 | ✅ |
...
| F. ダッシュボード | Streamlit画面で候補承認・日時予約・予約一覧・リード確認 | ✅（ローカル） |

## 投稿の流れ（承認モデル）

```
[6hごと] generate  → 候補をpending_reviewでキューに溜める＋Chatwork通知
[随時]   ダッシュボードで承認 → 投稿日時を指定（status=scheduled）
...
```bash
streamlit run app.py     # ブラウザで開く。候補の承認・予約・リード確認
```
...
- **凍結リスク対策**：返信は既定で**下書き承認制**。投稿/返信数も自主上限で絞っている（config.yamlの `safety`）。
