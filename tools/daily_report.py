...
RATE_COLS = {"1本あたり表示": ("ポスト表示回数", "ポスト数"),
             "コメント率": ("コメント数", "ポスト表示回数"),
             # ★診断率は【返信を送った数】で割る。コメント数やない。
             #   診断ページへ押しとるんはコメントやのうて、こっちが送った返信やからや。
             #   ★この率だけは100%を超える。固定投稿からの流入が混じるんで、
             #     「返信1本が何件の診断を生んだか」の目安として読む（下の定義タブ参照）。
             "診断率": ("診断完了数", "返信数"),
...
FORMATS = {"日": {"numberFormat": {"type": "DATE", "pattern": "yyyy-mm-dd"}},
           ...
           "返信数": _N, "診断完了数": _N, "診断率": _P, "LINE追加数": _N, "LINE追加率": _P,
...
LEGEND = [
    ...
    ["返信数", "その日にコメントへ送った返信", "draft_replies の sent_at", ""],
    ["診断完了数", "その日に診断を終えた人", "web_events の submit", ""],
    ["診断率", "診断完了数 ÷ 返信数", "式",
     "★コメント返信の効きを見る列。診断ページへ押しとるんは、こっちが送った返信やから分母は返信数や。"
     "★この列だけ100%を超える。固定投稿を直接見て診断する人も混じるんで、"
     "返信だけの成果に切り分けられん（web_diagのsourceにアカウントA/Bしか残っとらんため）。"
     "絶対値やのうて【動いたかどうか】で読む。返信の型を変えた前後で比べるのが正しい使い方や"],
    ["LINE追加数", "その日に友だち追加した人", "line_users の新規", ""],
    ["LINE追加率", "追加数 ÷ 診断完了数", "式", "診断からLINEへの落ち。数分の差やから同じ日で割ってええ"],
    ["", "★率が100%以下に収まる理由", "むりやり抑えとるんやない",
     "追加後の3つ（オファー・購入・ブロック）は【その日追加した人の中で数えとる】。"
     "分子が分母の一部やから、構造上100%を超えん。実際8/18は11人中11人でちょうど100%が出とる。"
     "★はみ出す分もある：追加から8日以上たってからのオファー20件(3%)・購入5件(5%)は、どの行にも入っとらん"],
...
def collect(d_from: str, d_to: str) -> list[dict]:
    ...
    rep = Counter()
    for r in ss._records("draft_replies"):
        d = str(r.get("sent_at") or "")[:10]
        if d and ok(d):
            rep[d] += 1
    ...
    while d.isoformat() <= d_to:
        ...
        out.append({"日": k, "ポスト数": posts[k], "表示": views[k], "コメ": com[k],
                    "返信": rep[k], "診断": diag[k], "追加": len(us), "オファー": n_off, "購入": n_buy,
...
def main() -> int:
    ...
    print(f"\n期間 {d_from} 〜 {d_to}")
    print("日付        投稿 表示回数 1本 コメ 返信 診断 追加 ｵﾌｧｰ 購入 ﾌﾞﾛｯｸ  ｺﾎｰﾄ売上  当日売上 確定")
    for r in rows:
        per = r["表示"] // r["ポスト数"] if r["ポスト数"] else 0
        # ★2026-09-13：見出しは「コメ 返信 診断」やのに、値は コメ→診断→返信 で出しとった。
        #   画面で読み違える（実際に一回読み違えた）。並びを見出しに合わせる。
        print(f"{r['日']} {r['ポスト数']:4d} {r['表示']:8,d} {per:4d} {r['コメ']:4d} {r['返信']:4d} "
              f"{r['診断']:4d} {r['追加']:4d} {r['オファー']:4d} {r['購入']:4d} {r['ブロック']:5d} "
